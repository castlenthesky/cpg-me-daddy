...
| **REJECT for v1, names reserved** | `CFG`, `CDG`, `DOMINATE`, `POST_DOMINATE`, `REACHING_DEF`, `CONDITION`, `EVAL_TYPE`, `CATCH_BODY`/`DO_BODY`/`FALSE_BODY`/`FINALLY_BODY`/`FOR_BODY`/`FOR_INIT`/`FOR_UPDATE`/`TRUE_BODY`/`TRY_BODY`, `JUMP_ARGUMENT` | — | CFG/PDG tier, deferred with D23/RK1. Standard compiler-theory names reserved unchanged for when these ship. |
