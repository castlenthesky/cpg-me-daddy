...
**Rules 1, 2 and 3 have been amended since this session** — see the per-rule note on each.
`50-schema.md` is now normative for the graph shape (node labels, edge types, id rule, status enums,
Joern divergence record); this list stays for the write-strategy and lifecycle rules (4-9), which are
still exactly as agreed.
...
1. **Stable, content-derived IDs** — `${fileHash}:${kind}:${scopePath}:${ordinal}`, never positions. Every CPG node carries its kind label **plus a shared `:CPG` label** with indexes on `CPG.id` and `CPG.file`.
   **SUPERSEDED 2026-09-11 by AM1/D31.** The id is `${path}:${kind}:${qualifiedScopePath}[:${ordinal}|:${bodyHash}]` — no file content hash, which would change every id in a file on every save. Normative text: `50-schema.md` §6. The rest of this rule (shared `:CPG` label, indexes on `CPG.id`/`CPG.file`, never positions) stands.
2. **SYMBOL indirection (SCIP model)** — `(:CALL)-[:CALLS]->(:SYMBOL {fqn})<-[:DEFINES]-(:METHOD)`. SYMBOL nodes are not `:CPG` and have no `file`, so a per-file delete cannot touch them; cross-file edges survive a replace by construction. Never create bare-name SYMBOLs for unresolved dynamic calls (store `resolved:false` + `calleeName` on the CALL). GC symbols per save, bounded to touched fqns. `UNIQUE SYMBOL.fqn` constraint as a tripwire.
   **AMENDED 2026-09-11 by R6.** The bare `resolved:false` boolean on the CALL is dropped — resolution status lives on the `CALLS`/`IMPORTS` edges as a shared five-value enum instead (one signal, one place; see `50-schema.md` §7). Everything else in this rule — SYMBOL indirection, no bare-name SYMBOLs, bounded GC, the `UNIQUE SYMBOL.fqn` tripwire — stands, untouched.
3. **Two-tier schema** — *structural* tier by default (FILE, NAMESPACE_BLOCK, TYPE_DECL, METHOD, PARAM, METHOD_RETURN, MEMBER, LOCAL, BLOCK, CONTROL_STRUCTURE, RETURN, CALL, SYMBOL; args/receiver/literals folded into CALL properties; REACHING_DEF from LOCAL/PARAM to CALL/RETURN). *Expression* tier (IDENTIFIER, LITERAL, FIELD_IDENTIFIER, REF, full REACHING_DEF) is opt-in. ~450 vs ~3,000 nodes per 300-line file. Drop `SOURCE_FILE`/`CONTAINS` edges — `file` is a property.
   **AMENDED 2026-09-11 by R6.** The tier principle and the measured density gap stand. The label list is superseded: `NAMESPACE_BLOCK` → `MODULE`; `METHOD_RETURN`, `BLOCK`, `CONTROL_STRUCTURE`, `RETURN`, `LOCAL` are not v1 labels (names reserved for the CFG tier); `IMPORT`, `TAG`, `UNKNOWN`, `META_DATA` added. Authoritative list: `50-schema.md` §3/§4. Also note the last sentence now reads backwards: `SOURCE_FILE` is indeed dropped, but `CONTAINS` was **renamed** to `HAS_ENTRY`, not dropped — the filesystem-nesting edge it named is kept, and `CONTAINS` itself is reserved unclaimed for Joern's opposite-meaning `METHOD -> CFG-node` edge.
