...
describe("FalkorDB store client", () => {
  test("writes nodes, reads them back, and satisfies the invariants", async () => {
    ...
    const files = await g.client.scalar("MATCH (f:FILE) RETURN count(f) AS n");
    expect(files).toBe(1);

    const symbols = await g.client.read<{ fqn: string }>(
    ...
    await assertGraphInvariants(g.client);
  });
  ...
  test("read() is read-only at the server, not by convention", async () => {
    ...
    await expect(g.client.read("CREATE (:SYMBOL {fqn:'nope'})")).rejects.toThrow(/read-only/i);
    const count = await g.client.scalar("MATCH (n) RETURN count(n) AS n");
    expect(count).toBe(0);
  });

  test("explain returns a plan", async () => {
    const g = await graph("explain");
    const plan = await g.client.explain("MATCH (s:SYMBOL {fqn:'x'}) RETURN s");
    expect(plan.join("\n")).toMatch(/Scan/i);
  });
  ...
  test("dropGraph removes the key and is safe to repeat", async () => {
    ...
    await seed(g);
    expect(await g.client.listGraphs()).toContain(g.key);
    await g.client.dropGraph();
    await g.client.dropGraph();
    expect(await g.client.listGraphs()).not.toContain(g.key);
  });
});
...
describe("graph invariants", () => {
  test("a duplicate SYMBOL fqn is caught", async () => {
    ...
    const result = await checkGraphInvariants(g.client);
    expect(result.ok).toBe(false);
    expect(result.problems.join("\n")).toMatch(/duplicate SYMBOL fqn/);
    await expect(assertGraphInvariants(g.client)).rejects.toThrow(/duplicate SYMBOL fqn/);
  });

  test("an orphan SYMBOL is caught", async () => {
    const g = await graph("orphan");
    await seed(g);
    await g.client.write("CREATE (:SYMBOL {fqn:'src/a.ts#stranded', file:'src/a.ts'})");
  ...
  test("a FILE left mid-replace is caught", async () => {
    ...
    await seed(g);
    await g.client.write("MATCH (f:FILE {path:'src/a.ts'}) SET f.status = 'updating'");
  ...
  test("repairing the graph makes the invariants green again", async () => {
    ...
    await seed(g);
    await g.client.write("CREATE (:SYMBOL {fqn:'src/a.ts#stranded'})");
    expect((await checkGraphInvariants(g.client)).ok).toBe(false);

    await g.client.write("MATCH (s:SYMBOL {fqn:'src/a.ts#stranded'}) DELETE s");
    await assertGraphInvariants(g.client);
  });
});
